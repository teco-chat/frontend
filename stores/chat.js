...
export const useChatStore = defineStore("chat", () => {
  const SOCKET_END_MESSAGE = "[DONE] - ID:";
  const query = ref("");
  ...
  const startWithChatId = async (id) => {
    if (load.value) {
      return;
    }
    load.value = true;
  ...
  const answer = async () => {
    ...
    ws.onmessage = function (event) {
      ...
      if (result.startsWith(SOCKET_END_MESSAGE)) {
        chatId.value = parseInt(result.replace(SOCKET_END_MESSAGE, ''));
        return;
      } 
      ...
      if (item.value.messages[lastIndexOfMessages()].content.match(codeFencesRegex)) {
        item.value.messages[lastIndexOfMessages()].content = replaceCodeFences(item.value.messages[lastIndexOfMessages()].content);
      }
    }
  ...
  return { query, load, item, question, answer, clear, clearAll, startWithChatId };
});
